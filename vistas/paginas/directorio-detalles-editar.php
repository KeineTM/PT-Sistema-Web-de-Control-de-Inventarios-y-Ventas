...
<form class="formulario" method="post" id="formulario-directorio">
    <fieldset class="formulario__fieldset">
        ...
        <input type="text" class="campo" placeholder="Apellido" name="apellido_materno-txt" autocomplete="off" data-form="apellido_materno" maxlength="80" value=<?= $consulta[0]['apellido_materno'] ?>>

        <label for="contacto_id_nuevo-txt">Teléfono:</label>
        <input type="number" step="any" class="campo requerido" placeholder="1234567890" name="contacto_id_nuevo-txt" autocomplete="off" data-form="contacto_id" minlength="10" maxlength="10" required value=<?= $consulta[0]['contacto_id'] ?>>
    ...
    <fieldset class="formulario__fieldset">
        ...
        <textarea name="notas-txt" class="campo requerido" autocomplete="off" cols="30" rows="3" data-form="notas" placeholder="Contador, Pizzería Frogs, Zaragoza..." maxlength="250" required><?= $consulta[0]['notas'] ?></textarea>
        ...
        <fieldset class="fieldset__envoltura formulario__fieldset-2-columnas">
            ...
            <input type="radio" name="tipo_contacto-txt" value="2" required data-form="tipo_id" <?php if ($consulta[0]['tipo_id'] === 2) echo 'checked' ?>>

            <label for="tipo_contacto-txt">Proveedor</label>
            <input type="radio" name="tipo_contacto-txt" value="1" required data-form="tipo_id" <?php if ($consulta[0]['tipo_id'] === 1) echo 'checked' ?>>

            <label for="tipo_contacto-txt">Servicios</label>
            <input type="radio" name="tipo_contacto-txt" value="3" required data-form="tipo_id" <?php if ($consulta[0]['tipo_id'] === 3) echo 'checked' ?>>
        </fieldset>
